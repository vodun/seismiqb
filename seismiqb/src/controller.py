...
from ..batchflow import B, V, C, F, D, P, R, W, L
...
from .models import ExtentionModel
...
EXTENTION_CONFIG = {'initial_block/inputs': ['images', 'prior_masks'],
                    'body/encoder': {'parallel': True,
                                       'num_stages': 3,
                    ...
                    'body/embedding': {'base':  ResNet.block,
                                        'filters': 128},
...
class EnhanceController(Controller):
    """ !!."""
    #pylint: disable=unused-argument
    def __init__(self, batch_size=64, crop_shape=(1, 256, 256),
                 model_config=None, model_path=None, device=None,
    ...
    def enhance_train(self, dataset, n_epochs=300, use_grid=False, side_view=True,
                      horizon=None):
        """ !!. """
        if horizon:
            dataset = self.update_labels(dataset, horizon, idx=0, dst='labels')
            self.make_sampler(dataset, side_view=0.5, use_grid=False)

        if self.model_path is None:
            # self.logger('Train started')
            in_channels = self.crop_shape[0]
            pipeline_config = {
                'model_config': {**(self.model_config or EXTENTION_CONFIG),
                                 'inputs/images/shape': (None, None, in_channels),
                                 'inputs/masks/shape': (None, None, in_channels, 1),
                                 'inputs/prior_masks/shape': (None, None, in_channels),                
                                 'device': self.device
                                },
                'make_slices': 'adaptive' if use_grid is True else True,
                'side_view': side_view,
            }
            train_pipeline = (self.get_enhance_train_template() << pipeline_config) << dataset
    ...
    def get_enhance_train_template(self):
        """ !!. """
        # model_config
        # crop_shape
        # make_slices
        print('in get_enhance_train_template')
        def functor(scale):
        ...
        train_template = (
            Pipeline()
                ...
                .create_masks(dst='masks', width=3)
                .adaptive_reshape(src=['images', 'masks'], shape=self.crop_shape)
                .mask_rebatch(src='masks', threshold=0.9, passdown=None)
                .rotate_axes(src=['images', 'masks'])
                .scale(mode='q', src='images') + 

            # Cut out masks for model training
            Pipeline()
            ...
            Pipeline()
                .filter_out(src='masks', dst='prior_masks',
                            expr=L(functor)(R('uniform', low=15, high=35)),
                            low=0.0) @ 0.7 +

            Pipeline()
#                 Augmentations
                .flip(axis=1, src=['images', 'masks', 'prior_masks'], p=0.3)
                .additive_noise(scale=0.005, src='images', dst='images', p=0.3)
                .rotate(angle=P(R('uniform', -15, 15)),
                ...
                .scale_2d(scale=P(R('uniform', 0.85, 1.15)),
                        src=['images', 'masks', 'prior_masks'], p=0.3)
                .elastic_transform(alpha=P(R('uniform', 35, 45)), sigma=P(R('uniform', 4, 4.5)),
                                src=['images', 'masks', 'prior_masks'], p=0.2)
                # Training
                .add_axis(src='masks')
                .init_variable('loss_history', default=list())
                .init_model('dynamic', ExtentionModel, 'base', C('model_config'))
                .train_model('base', fetches='loss', save_to=V('loss_history', mode='a'),
                            images=B('images'), masks=B('masks'), prior_masks=B('prior_masks'))
            )
        return train_template

    # Load predicted horizons to dataset attribute
    def update_labels(self, dataset, horizons, idx=0, dst='predicted_labels'):
        """ !!. """
        setattr(dataset, dst, IndexedDict())
        getattr(dataset, dst)[dataset.indices[idx]] = [horizons]
        return dataset

    def get_enhance_inference_template(self, labels_src):
        inference_template = (
            Pipeline()
                .crop(points=L(D('crops_gen')), shape=L(D('shapes_gen')),
                      passdown=labels_src)
                .load_cubes(dst='images')
                .create_masks(dst='prior_masks', width=3, src_labels=labels_src)
                .adaptive_reshape(src=['images', 'prior_masks'], shape=self.crop_shape)
                .rotate_axes(src=['images', 'prior_masks'])
                .scale(mode='normalize', src='images')
                .import_model('base',  C('train_pipeline'))
                .init_variable('predicted_horizons', default=list())
                .predict_model('base', fetches='predictions',
                                images=B('images'), prior_masks=B('prior_masks'),
    ...
    def enhance_inference(self, dataset, n_steps, horizon, batch_size=200):
        """ !!. """
        if horizon:
            dataset = self.update_labels(dataset, horizon, idx=0, dst='predicted_labels')
        ...
        for i in range(n_steps):
            dataset.make_expand_grid_v2(dataset.indices[0], crop_shape=self.crop_shape, stride=10,
                                        labels_src='predicted_labels', batch_size=batch_size)
            predict_ppl = (self.get_enhance_inference_template('predicted_labels') << pipeline_config) << dataset

            for i in self.make_pbar(range(dataset.crops_iters), desc=f'Enhance on {horizon.name} step {i}'):
            ...
            for hor in merge_candidates:
                merge_code, _ = Horizon.verify_merge(dataset.predicted_labels[dataset.indices[0]][0], hor,
                                                     mean_threshold=16.5,
                                                     adjacency=5)
                if merge_code in [2, 3]:
                    _ = dataset.predicted_labels[dataset.indices[0]][0].overlap_merge(hor, inplace=True)

        setattr(self, f'enhanced_{horizon.name}', dataset.predicted_labels[dataset.indices[0]][0])
        return dataset.predicted_labels[dataset.indices[0]][0]