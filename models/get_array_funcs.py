...
def make_test_config():
    ...
    horizon_matrix = np.tile((125, 126, 127, 128, 129, 130), (1000, 300))
    args = (horizon_matrix, horizon_offset, array, array_offset, width, fill_value)
    return args


def make_func(loop=True, memory_profile=True):
    if loop:
        def func(horizon_matrix, horizon_offset, array, array_offset, width, fill_value):
        ...
        def func(horizon_matrix, horizon_shift, array, array_shift, width=5, fill_value=-999):
...
def make_test_func(func, memory_profile=True):
    def result_func():
        args = make_test_config()
        return func(*args)

    if memory_profile:
        result_func = profile(result_func)
